...
export const allergyRule = (ctx: MatchingContext) => {
  ...
  if (
    ...
    adopter.specific_animal_allergies?.includes('bird' | 'cat' | 'dog') &&
    pet.species === adopter.specific_animal_allergies

  ) {
    return { rejected: true, reason: 'Allergic to ' + adopter.specific_animal_allergies };
  }
...
export const noTimeRule = (ctx: MatchingContext) => {
  if (ctx.adopter.daily_care_time === 0) {
    return { rejected: true, reason: 'No time for pet care' };
  }
